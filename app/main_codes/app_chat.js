...
function dispChatDtl(){ 
  ...
  for(var i=0;i<aryChat.length;i++){    
    ...
    var v_userImg=JBE_API+'upload/users/'+JBE_GETFLD('photo',DB_CLIENTS,'usercode',v_usercode)+'?'+n;
    if(CURR_AXTYPE>0){ 
      if(v_sender==1){      
...
function delChat(v_trano){  
  var usercode=document.getElementById('div_main_chat').getAttribute('data-usercode');
  var f_owner=false;
  var ddir=CURR_SITE+'/upload/chat/';
  //alert(v_trano);
  //alert(ddir);
  if(CURR_AXTYPE > 0){ f_owner=true; }
...
function chat_owner(){
  ...
  JBE_OPEN_VIEW(dtl,'Select Client','close_chat_owner');
  disp_chat_owner();
}
